...
7. **Update PRD** — edit `PRD.md` to change the Status of the implemented user story from `Planned` to `Done` in the markdown table row matching the story ID, then commit with `/commit`
8. **Merge & cleanup** — **ONLY after the user explicitly approves**, stop the dev server, then use the `/worktree` skill to merge the branch into main and remove the worktree
